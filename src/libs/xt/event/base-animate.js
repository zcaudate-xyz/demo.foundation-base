...
import * as xtm from '../lang/common-math.js'

// xt.event.base-animate/new-derived [10] 
export function new_derived(impl,f,arr){
...
// xt.event.base-animate/listen-single [31] 
export function listen_single(impl,ref,ind,f){
  if(null == f){
    f = (function (_){
      return {};
    });
...
// xt.event.base-animate/listen-array [52] 
export function listen_array(impl,ref,arr,f){
  if(null == f){
    f = (function (_){
      return {};
    });
...
// xt.event.base-animate/get-map-paths-inner [75] 
export function get_map_paths_inner(impl,m,parent,all){
  if(null == parent){
    parent = [];
  }
  if(null == all){
...
// xt.event.base-animate/get-map-paths [98] 
export function get_map_paths(impl,m){
  return get_map_paths_inner(impl,m,[],[]);
}

// xt.event.base-animate/get-map-input [104] 
export function get_map_input(impl,paths){
  let {get_value} = impl;
  let out = {};
  for(let e of paths){
    let [path,key,v] = e;
...
// xt.event.base-animate/listen-map [128] 
export function listen_map(impl,ref,m,f){
  if(null == f){
    f = (function (_){
      return {};
    });
...
// xt.event.base-animate/listen-transformations [155] 
export function listen_transformations(impl,ref,indicators,transformations,get_chord){
...
// xt.event.base-animate/new-progressing [185] 
export function new_progressing(){
  return {"running":false,"animation":null,"queued":[]};
}

// xt.event.base-animate/run-with-cancel [194] 
export function run_with_cancel(impl,animate_fn,progressing,progress_fn){
...
// xt.event.base-animate/animate-chained-cleanup [223] 
export function animate_chained_cleanup(impl,progressing,progress_fn){
...
// xt.event.base-animate/animate-chained-one [234] 
export function animate_chained_one(impl,progressing,progress_fn){
...
// xt.event.base-animate/animate-chained-all [258] 
export function animate_chained_all(impl,progressing,progress_fn){
  let {queued} = progressing;
  if(0 == queued.length){
    return animate_chained_cleanup(impl,progressing,progress_fn);
  }
...
// xt.event.base-animate/run-with-chained [278] 
export function run_with_chained(impl,type,animate_fn,progressing,progress_fn){
...
// xt.event.base-animate/run-with [306] 
export function run_with(impl,type,animate_fn,progressing,progress_fn){
  if(type == "cancel"){
...
// xt.event.base-animate/make-binary-transitions [324] 
export function make_binary_transitions(impl,initial,tparams){
  if(null == tparams){
    tparams = {};
  }
...
// xt.event.base-animate/make-binary-indicator [356] 
export function make_binary_indicator(impl,initial,tparams,type,progressing,progress_fn){
  if(null == tparams){
    tparams = {};
  }
...
// xt.event.base-animate/make-linear-indicator-inner [385] 
export function make_linear_indicator_inner(impl,initial,get_prev,set_prev,tparams,type,progressing,progress_fn,check_fn){
  let {create_transition,create_val} = impl;
...
// xt.event.base-animate/make-linear-indicator [421] 
export function make_linear_indicator(impl,initial,get_prev,set_prev,tparams,type,progressing,progress_fn){
...
// xt.event.base-animate/make-circular-indicator-inner [442] 
export function make_circular_indicator_inner(impl,initial,get_prev,set_prev,tparams,type,modulo,progressing,progress_fn,check_fn){
  let {create_transition,create_val} = impl;
  let indicator = create_val(initial);
  let trigger_fn = function (value){
    let should_run = true;
...
// xt.event.base-animate/make-circular-indicator [485] 
export function make_circular_indicator(impl,initial,get_prev,set_prev,tparams,type,modulo,progressing,progress_fn){
