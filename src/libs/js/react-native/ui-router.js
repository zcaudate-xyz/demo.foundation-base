...
// js.react-native.ui-router/useTransition [20] 
export function useTransition({indicators,chord,indicatorParams = {},onComplete}){
...
// js.react-native.ui-router/RouterImpl [54] 
export function RouterImpl({
  ...
  transition = (transition || xtd.get_in(transitionMap,[prevKey,routeKey]) || transitionDefault);
  ...
  let outFn = model_context.animateOut(Object.assign({},chord,{
    "transition":xtd.get_in(model_context.ANIMATE,[transition,"counter"])
  }));
  let evenFn = function (progress,chord){
    let {changing,even} = chord;
    return changing ? {
      "style":even ? {"transform":xtd.arrayify(outFn(progress))} : {"transform":xtd.arrayify(inFn(progress))}
    } : null;
  };
  ...
  React.useEffect(function (){
    n.measureRef(containerRef,function (e){
      if(xtd.not_emptyp(xtd.obj_difference(e,dims))){
  ...
  return (
    <React.Fragment>
          ...
          inner={[
              {
                ...
                "children":[
                    (even || changing) ? (
                      <n.GlobalProvider key="even" value={{"isTransition":changing}}><EvenComponent {...evenProps}/></n.GlobalProvider>) : null
                  ],
                "transformations":{
              ...
              },
              {
                ...
                "children":[
                    (!even || changing) ? (
                      <n.GlobalProvider key="odd" value={{"isTransition":changing}}><OddComponent key="odd" {...oddProps}/></n.GlobalProvider>) : null
                  ]
              }
            ]}/>
      </ReactNative.View>
      {debug ? (
        <n.TextDisplay
          content={n.format_entry({changing,even,evenKey,oddKey,prevKey,routeKey,transition})}/>) : null}
    </React.Fragment>);
}

// js.react-native.ui-router/Router [200] 
